    ...
    >
      <div className="pb-5">
        <div className="flex justify-between items-center">
          <img className="max-w-25" src={assets.logo} alt="" />
      ...
      <div className="flex flex-col">
        {
            userDummyData.map((user,index) => (
                <div onClick={() => {setSelectedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm
                    ${selectedUser?._id === user._id && 'bg-[#282142/50'
                }`} >
                    <img className="w-[35px] aspect-square rounded-full" src={user?.profilePic || assets.avatar_icon} alt="" />
